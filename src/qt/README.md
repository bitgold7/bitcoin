This directory contains the source code for the Bitgold Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bitgold UI.
...
- ClientModel is used by the main application `bitgoldgui` and several models like `peertablemodel`.
...
* `bitgoldstrings.cpp`: automatically generated
...
4. Enter `bitgold-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitgold-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `bitgold-qt`
9. You're all set! Start developing, building, and debugging the Bitgold Core GUI
