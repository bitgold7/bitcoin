...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
     ...
     * (to be consistent with bitgoldd behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    // Expected block interval (in seconds). Sourced from chainparams, but hard-coded
    // here as consensus parameters are not yet available at this stage.
    static constexpr uint64_t nPowTargetSpacing = 8 * 60; // 8-minute blocks
    static constexpr uint32_t expected_block_data_size = 2250000;  // includes undo data
    const uint64_t expected_backup_days = m_prune_target_gb * 1e9 /
        (uint64_t(expected_block_data_size) * 86400 / nPowTargetSpacing);
    ui->lblPruneSuffix->setText(
        //: Explanatory text on the capability of the current prune target.
        tr("(sufficient to restore backups %n day(s) old)", "", expected_backup_days));
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the BitGold block chain.").arg(CLIENT_NAME) + " " +
