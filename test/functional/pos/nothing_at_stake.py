"""Simulate a nothing-at-stake scenario with duplicate staking."""
...
class PosNothingAtStakeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block1 = build_block(ntime)
        node.submitblock(block1.serialize().hex())
        assert_equal(node.getblockcount(), prev_height + 1)

        block2 = build_block(ntime + 16)
        assert node.submitblock(block2.serialize().hex()) is not None
        assert_equal(node.getblockcount(), prev_height + 1)


if __name__ == "__main__":
    PosNothingAtStakeTest(__file__).main()
