...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from bitgold-build-config.h is used without the header being included",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/bitgold-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <bitgold-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the bitgold-build-config.h header. However, they are not
...
even though bitgold-build-config.h indicates that a faster feature is available and should be used.
...
#include <bitgold-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the bitgold-build-config.h header. However, they are including
