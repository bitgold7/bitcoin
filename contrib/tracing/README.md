...
The bpftrace examples contain a relative path to the `bitgoldd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bitgoldd` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof bitgoldd)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof bitgoldd)
```
...
```
$ ./build/bin/bitgoldd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof bitgoldd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof bitgoldd)
```
