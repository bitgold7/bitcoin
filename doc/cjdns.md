...
CJDNS peers than Tor or IP ones. You can use `bitgold-cli -addrinfo` to see the
...
To see which CJDNS peers your node is connected to, use `bitgold-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitgold-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitgold-cli help getnodeaddresses` for details.
