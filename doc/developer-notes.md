...
configuration options and toggled while bitgoldd is running with the `logging`
RPC.  For instance, launching bitgoldd with `-debug` or `-debug=1` will turn on
...
You can then use the `-debug=lock` configuration option at bitgoldd startup or
`bitgold-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bitgold-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full build/bin/bitgoldd -printtoconsole
...
To profile a running bitgoldd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bitgoldd` -- sleep 60
```
...
- [Main thread (`bitgoldd`)](https://doxygen.bitcoincore.org/bitgoldd_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `bitgoldd.cpp`. Responsible for starting up and
...
- Use the JSON parser for parsing, don't manually parse integers or strings from
  ...
  - *Exception*: AmountFromValue can parse amounts as string. This was introduced because many JSON
    ...
    `prioritisetransaction` because its interface is specified as-is.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitgold-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitgold-cli` and the GUI debug console use this table to determine how to
