...
The headless daemon `bitgoldd` has the JSON-RPC API enabled by default, the GUI
...
This is the endpoint used by bitgold-cli when a `-rpcwallet=` parameter is passed in.
...
bitgold-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
bitgold-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
bitgold-cli -named createwallet mywallet load_on_startup=true
```

`bitcoin rpc` can also be substituted for `bitgold-cli -named`, and is a newer alternative.
...
- **Securing remote network access:** You may optionally allow other
  ...
  `bitgoldd -help` for more information about these settings and other
