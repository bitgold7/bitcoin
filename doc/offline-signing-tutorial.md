...
```sh
[offline]$ ./build/bin/bitgold-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/bin/bitgold-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -named createwallet \
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" getnewaddress

sb1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajg0h6yh
```
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" listunspent

[
  {
    "txid": "0f3953dfc3eb8e753cd1633151837c5b9953992914ff32b7de08c47f1f29c762",
    "vout": 1,
    "address": "sb1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajg0h6yh",
    "label": "",
...
1. Get a destination address for the transaction. In this tutorial we'll be sending funds to the address `sb1q9k5w0nhnhyeh78snpxh0t5t7c3lxdeg3erez32`, but if you don't need the coins for further testing you could send the coins back to the faucet.
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" send \
              '{"sb1q9k5w0nhnhyeh78snpxh0t5t7c3lxdeg3erez32": 0.009}' \
              | jq -r '.psbt' \
...
```sh
[offline]$ ./build/bin/bitgold-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/bitgold-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/bitgold-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/bitgold-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/bitgold-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/bitgold-cli -signet -rpcwallet="watch_only_wallet" listtransactions
