...
```sh
$ bitgoldd -signer=../HWI/hwi.py
```

`bitcoin node` can also be substituted for `bitgoldd`.
...
```sh
$ bitgold-cli createwallet "hww" true true "" true true true
```
...
```sh
$ bitgold-cli -rpcwallet=<wallet> getnewaddress
$ bitgold-cli -rpcwallet=<wallet> walletdisplayaddress <address>
```
...
```sh
$ bitgold-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
```
