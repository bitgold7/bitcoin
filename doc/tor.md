...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `bitgold-cli help getnodeaddresses` for details.
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    bitgoldd -proxy=127.0.0.1:9050

`bitcoin node` or `bitcoin gui` can also be substituted for `bitgoldd`.
...
To see verbose Tor information in the bitgoldd debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or bitgoldd's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running bitgoldd must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` bitgoldd configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
bitgoldd to the same Tor group and setting permissions appropriately.
...
your bitgoldd's P2P listen port (8333 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    bitgoldd -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    bitgoldd ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    bitgoldd -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover
