...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitgoldd -help` output for these two options for
...
- The `testnet` field in `bitgold-cli -getinfo` has been renamed to
...
- #16366 init: Use InitError for all errors in bitgoldd/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of bitgoldd build (NicolasDorier)
...
- #16489 log: harmonize bitgoldd logging (jonatack)
...
- #16912 Remove Doxygen intro from src/bitgoldd.cpp (ch4ot1c)
