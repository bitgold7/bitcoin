...
```
bitgoldd -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `bitgold-cli help
...
`bitgold-cli -addrinfo` to see the number of I2P addresses known to your node.
...
To see which I2P peers your node is connected to, use `bitgold-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `bitgold-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `bitgold-cli help getnodeaddresses` for details.
